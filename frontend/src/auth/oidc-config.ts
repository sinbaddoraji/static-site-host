...
export const userManager = new UserManager({
  authority: getEnvVar('VITE_OIDC_AUTHORITY', 'https://sso.garri.ovh/'),
  client_id: getEnvVar('VITE_OIDC_CLIENT_ID', '312365402212597763'),
  client_secret: getEnvVar('VITE_OIDC_CLIENT_SECRET'),
  redirect_uri: buildUrl(getEnvVar('VITE_OIDC_REDIRECT_URI', '/callback')),
  response_type: getEnvVar('VITE_OIDC_RESPONSE_TYPE', 'code'),
  scope: getEnvVar('VITE_OIDC_SCOPE', 'openid profile email'),
  post_logout_redirect_uri: buildUrl(getEnvVar('VITE_OIDC_POST_LOGOUT_REDIRECT_URI', '/')),
  silent_redirect_uri: buildUrl(getEnvVar('VITE_OIDC_SILENT_REDIRECT_URI', '/silent-renew.html')),
  automaticSilentRenew: getEnvVar('VITE_OIDC_AUTOMATIC_SILENT_RENEW', 'true') === 'true',
  loadUserInfo: getEnvVar('VITE_OIDC_LOAD_USER_INFO', 'true') === 'true',
